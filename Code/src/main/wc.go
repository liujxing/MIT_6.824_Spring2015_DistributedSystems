...
func Map(value string) *list.List {
	valueSplit := strings.FieldsFunc(value, func(r rune) bool {
		return !unicode.IsLetter(r)
	})
	result := new(list.List)
	for _, token := range valueSplit {
		result.PushBack(mapreduce.KeyValue{Key:token, Value:"1"})
	}
	return result
}
...
func Reduce(key string, values *list.List) string {
	...
	for iterator := values.Front(); iterator != nil; iterator = iterator.Next() {
		// todo: handle the error from Atoi
		count, _ := strconv.Atoi(iterator.Value.(string))
		result += count
	}
